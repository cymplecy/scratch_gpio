...
class Compass:
    ...
    def degrees(self, xxx_todo_changeme):
        (degrees, minutes) = xxx_todo_changeme
        return str(degrees) + "*" + str(minutes) + "'"

    def degreesdecimal(self, xxx_todo_changeme1):
        (degrees, minutes) = xxx_todo_changeme1
        return str(degrees + (minutes / 60.0)) if (degrees >=
                                                   0) else str(degrees - (minutes / 60.0))
...
class ScratchSender(threading.Thread):
    ...
    def run(self):
        ...
        if "pipiano" in ADDON:
            with lock:
                for digitalIO in range(0, 13):
                    if (sghGC.piAndBash[digitalIO] in [
                            sghGC.PINPUT, sghGC.PINPUTNONE, sghGC.PINPUTDOWN]):
                        try:
        ...
        while not self.stopped():
                ...
                for listIndex in range(len(sghGC.validPins)):
                    ...
                    if (sghGC.pinUse[pin] in [
                        ...
                        if pinEvent:
                            ...
                            if pin_bit_pattern[
                                    listIndex] == last_bit_pattern[listIndex]:
                                logging.debug(
            ...
            if (pcfSensor is not None) and (
                    "explorer" not in ADDON):  # if PCF ADC found
                sensor_str = ""
                if tick % 5 == 0:
                ...
                with lock:
                    ...
                    for digitalIO in range(0, 8):
                        if (sghGC.piAndBash[digitalIO] in [
                                sghGC.PINPUT, sghGC.PINPUTNONE, sghGC.PINPUTDOWN]):
                            if mcp.input(
                                    digitalIO) != lastpiAndBash[digitalIO]:
                                    ...
                                    if (mcp.input(digitalIO)
                                        ...
                                        bcast_str += ' "' + sensor_name + '" ' + str(
                                            [
                                                ...
                                                66,
                                                68,
                                                70][digitalIO]) + " "  # str(mcp.input(digitalIO)>>digitalIO) + " "
                                    msgQueue.put(
            ...
            if wii is not None:  # if wii  found
                ...
                try:
                    ...
                    if (joyx == 255) or (joyy == 255) or (
                            accelx == 255) or (accely == 255) or (accelz == 255):
                        print dt.datetime.now()
            ...
            for listIndex in range(len(sghGC.validPins)):
                pin = sghGC.validPins[listIndex]
                if (pin_bit_pattern[listIndex] != last_bit_pattern[
                        listIndex]) or sghGC.pinTrigger[pin] == 1:
...
class ScratchListener(threading.Thread):
    ...
    def bFindOn(self, searchStr):
        return (self.bFind(searchStr + 'on ')
                or self.bFind(searchStr + 'high ') or self.bFind(searchStr + '1 '))

    def bFindOff(self, searchStr):
        return (self.bFind(searchStr + 'off ')
                or self.bFind(searchStr + 'low ') or self.bFind(searchStr + '0 '))
    ...
    def bCheckAll(self, default=True, pinList=None):
        if self.bFindOnOff('all'):
            ...
            for pin in pinList:
                # print pin
                if sghGC.pinUse[pin] in [
                        sghGC.POUTPUT, sghGC.PPWM, sghGC.PPWMMOTOR]:
                    # print pin
    ...
    def vFindOn(self, searchStr):
        return (self.vFind(searchStr + 'on')
                or self.vFind(searchStr + 'high') or self.vFind(searchStr + '1'))

    def vFindOff(self, searchStr):
        return (self.vFind(searchStr + 'off')
                or self.vFind(searchStr + 'low') or self.vFind(searchStr + '0'))
    ...
    def vPinCheck(self):
        for pin in sghGC.validPins:
            ...
            if self.vFindValue('powergpio' + str(sghGC.gpioLookup[pin])):
                ...
                logging.debug(
                    "gpiopower lookup %s", str(
                        sghGC.gpioLookup[pin]))
                if self.valueIsNumeric:
                    sghGC.pinUpdate(pin, self.valueNumeric, type="pwm")
                else:
                    sghGC.pinUpdate(pin, 0, type="pwm")
    ...
    def stepperUpdate(self, pins, value, steps=2123456789, stepDelay=0.003):
        ...
        try:
            ...
            sghGC.pinRef[
                pins[0]] = sgh_Stepper.sghStepper(
                sghGC,
                pins,
                stepDelay)  # create new Stepper instance
            sghGC.pinRef[pins[0]].changeSpeed(
    ...
    def moveMotor(self, motorList, count, pin):
        ...
        if count >= 0:
            ...
            while ((sghGC.pinCount[pin] < int(countattempted)) and (
                    (time.time() - turningStartTime) < 20)):
                if pin == 13:
    ...
    def beep(self, pin, freq, duration):
        ...
        sghGC.pinUpdate(
            pin,
            50,
            "pwm")  # Set duty cycle to 50% to produce square wave
            ...
            try:
                ...
                if len(data) > 0:
                    ...
                    while len(dataIn) > 0:  # loop thru data
                        ...
                        size = struct.unpack(
                            ">L",
                            sizeInfo)[0]  # get size of Scratch msg
                        # print "size:", size
                        if size > 0:
                            # print dataIn[4:size + 4]
                            dataMsg = dataIn[
                                4:size +
                                4].lower()  # turn msg into lower case
                            # print "msg:",dataMsg
                            ...
                            if len(dataMsg) == size:
                                if "alloff" in dataMsg:
                                    ...
                                    logging.debug(
                                        "not sure why this code is here Whole message:%s",
                                        dataIn)
                            ...
                            dataIn = dataIn[
                                size +
                                4:]  # cut data down that's been processed
            ...
            for dataItem in dataList:
                    ...
                    if pinsoraddon is not None:
                        ...
                        if "piringo" in ADDON:
                            with lock:
                                ...
                                piringoInputs = [
                                    19,
                                    21]  # These are the pins connected to the switches
                        ...
                        if "p2g3" in ADDON:
                            with lock:
                                ...
                                try:
                                    for i in range(
                                            0, 16):  # go thru PowerPWM on PCA Board
                        ...
                        if "pi2go" in ADDON:
                            if "pi2golite" in ADDON:
                                ...
                                with lock:
                                    ...
                                    try:
                                        for i in range(
                                                0, 12):  # go thru PowerPWM on PCA Board
                                            pcaPWM.setPWM(i, 0, 0)
                                    except:
                        ...
                        if "pitt" in ADDON:
                            with lock:
                                ...
                                for pin in [8, 10, 12, 16, 18, 22, 24, 26]:
                                    sghGC.pinUpdate(
                                        pin,
                                        1)  # turn all the pins physically off
                                anyAddOns = True
                        ...
                        if "piandbash" in ADDON:
                            ...
                            mcp.config(
                                14,
                                mcp.INPUT)  # LCD Upper Line Select Button
                            mcp.pullup(14, 1)
                            mcp.config(
                                15,
                                mcp.INPUT)  # LCD Lower Line Select Button
                            mcp.pullup(15, 1)
                        ...
                        if "explorer" in ADDON:
                            with lock:
                                sghGC.resetPinMode()
                                for pin in [
                                        7, 11, 13, 29, 31, 32, 33, 36, 40, 37, 38, 35]:
                                    sghGC.pinUse[pin] = sghGC.POUTPUT
                                ...
                                for pin in [
                                    ...
                                    sghGC.pinUpdate(
                                        pin,
                                        0)  # turn all the pins physically off
                                anyAddOns = True
                        print "anyAddOns:",anyAddOns
                        for pin in sghGC.validPins:
                            if (sghGC.pinUse[pin] in [
                                    sghGC.PINPUT, sghGC.PINPUTNONE, sghGC.PINPUTDOWN]):
                ...
                if 'sensor-update' in self.dataraw:
                            ...
                            if self.valueIsNumeric:
                                ...
                                for pin in [8, 10, 12, 16, 18, 22, 24, 26]:
                                    ...
                                    if bit_pattern[-(j + 1)] == '0':
                                        sghGC.pinUpdate(
                                            pin,
                                            1)  # output inverted as board is physically active low
                                    else:
                                        sghGC.pinUpdate(pin, 0)
                                    j += 1
                    ...
                    if self.vFindValue("matrixuse"):
                        self.matrixUse = int(
                            self.valueNumeric) if self.value in [
                            '64',
                            '16',
                            '9',
                            '4'] else 64
                    ...
                    # RGB-LED by Meltwater/rsstab/tim cox
                        ...
                        for listLoop in rgbList:
                            if self.vFindOnOff(
                                    "led" + str(1 + rgbList.index(listLoop))):
                                sghGC.pinUpdate(
                                    rgbOutputs[
                                        rgbList.index(listLoop)],
                                    1 -
                                    self.valueNumeric)
                            if self.vFindValue(
                                    "power" + str(1 + rgbList.index(listLoop))):
                                if self.valueIsNumeric:
                                    sghGC.pinUpdate(
                                        rgbOutputs[
                                            rgbList.index(listLoop)],
                        ...
                        if (pcaPWM is not None):
                            ledList = [0, 3, 6, 9, 12]
                            for i in range(
                                    0, 5):  # go thru PowerPWM on PCA Board
                                if self.vFindValue('blue'):
                                    svalue = int(
                                        self.valueNumeric) if self.valueIsNumeric else 0
                                    svalue = min(
                                        4095, max(
                                            (((100 - svalue) * 4096) / 100), 0))
                        ...
                        if (pcaPWM is not None):
                            ...
                            for i in range(
                                    12, 16):  # go thru servos on PCA Board
                        ...
                        if steppersInUse:
                            ...
                            for listLoop in range(0, 2):
                                if self.vFindValue(stepperList[listLoop][0]):
                                    logging.debug(
                                        "Stepper found %s",
                                        stepperList[listLoop][0])
                                    if self.valueIsNumeric:
                                        self.stepperUpdate(
                                            stepperList[listLoop][1],
                                            self.valueNumeric)
                                    else:
                                        self.stepperUpdate(
                                            stepperList[listLoop][1],
                    ...
                    if pcaPWM is not None:
                        ...
                        if self.vFindValue('mehorizontal'):
                            meHorizontal = max(-50,
                                               min(50,
                                                   int(self.valueNumeric))) if self.valueIsNumeric else 0
                            meArmAction = True
                    ...
                    if "mearm" in ADDON:
                        ...
                        if (pcaPWM is not None):
                            ...
                            for i in range(
                                    0, 16):  # go thru servos on PCA Board
                                if self.vFindValue('servo' + str(i)):
                                    svalue = int(
                                        self.valueNumeric) if self.valueIsNumeric else 0
                                    # print i, svalue
                ...
                if 'broadcast' in self.dataraw:
                    ...
                    if self.bFindValue("touchreset"):
                        ...
                        if self.value == "":
                            ...
                            for loop in range(0, 8):
                                if sghCT.ctTrigStatus[loop][
                                        0] == int(self.valueNumeric):
                                    sghCT.ctTrigStatus[loop][1] = 0
                                    sensor_name = "touch" + \
                                        str(sghCT.ctTrigStatus[loop][0])
                                    sensor_value = "0"
                    ...
                    if "ladder" in ADDON:  # Gordon's Ladder Board
                        ...
                        for i in range(len(pcolours)):
                            if self.bFindOnOff(pcolours[i]):
                                ...
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        i + 0]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        i + 6]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        i + 12]] = PiGlow_Brightness * self.OnOrOff
                                piglow.update_pwm_values(PiGlow_Values)
                        ...
                        for i in range(1, 4):
                            if self.bFindOnOff('leg' + str(i)):
                                ...
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        ((i - 1) * 6) + 0]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        ((i - 1) * 6) + 1]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        ((i - 1) * 6) + 2]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        ((i - 1) * 6) + 3]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        ((i - 1) * 6) + 4]] = PiGlow_Brightness * self.OnOrOff
                                PiGlow_Values[
                                    PiGlow_Lookup[
                                        ((i - 1) * 6) + 5]] = PiGlow_Brightness * self.OnOrOff
                                piglow.update_pwm_values(PiGlow_Values)
                        ...
                        self.bCheckAll()
                        self.bPinCheck(
                            sghGC.validPins)  # Check for pin off/on type broadcasts
                        ...
                        if self.bFindValue("movea"):
                            ...
                            msgQueue.put(
                                (5, 'sensor-update "motors" "turning"'))  # set turning sensor to turning
                            time.sleep(0.2)
                            moveFound = True
                            print " "
                            svalue = int(
                            ...
                            sghGC.pinLastState[
                                motorList[0][3]] = -1  # sghGC.pinRead(motorList[0][3])
                        ...
                        if self.bFindValue("moveb"):
                            ...
                            sghGC.pinLastState[
                                motorList[1][3]] = -1  # sghGC.pinRead(motorList[0][3])
                        ...
                        if (pcaPWM is not None):
                            for i in range(
                                    0, 4):  # go thru PowerPWM on PCA Board
                                if self.bFindValue('blue'):
                                    svalue = int(
                                        self.valueNumeric) if self.valueIsNumeric else 100 if self.value == "on" else 0
                                    svalue = svalue * sghGC.ledDim / 100
                        ...
                        def pi2go_mapName(name):
                            print name
                            try:
                                return ['left', 'back', 'right', 'front'].index(
                                    name) + 1
                            except:
                                return 0
                        ...
                        if self.bFind("pixel"):
                            ...
                            for led in range(0, self.matrixUse):
                                if (self.bFindValue("pixel") and pi2go_mapName(
                                        self.value) == str(led + 1)):
                                    set_neopixel(
                                        led,
                                        self.matrixRed,
                                        self.matrixGreen,
                            ...
                            if not pixelProcessed:
                                for led in range(0, self.matrixUse):
                                    for ledcolour in ledcolours:
                                        if (self.bFindValue("pixel", ledcolour)) and pi2go_mapName(
                                                self.value) == str(led + 1):
                                            self.matrixRed, self.matrixGreen, self.matrixBlue = tcolours.get(
                            ...
                            if "unicorn" in ADDON:
                                if self.bFind("pixel"):
                                    ...
                                    if not pixelProcessed:
                                        for led in range(0, self.matrixUse):
                                            if (self.bFindValue(
                                                    "pixel") and self.value == str(led + 1)):
                                                ym = int(
                                                    int(led) /
                                                    self.matrixRangemax)
                                    ...
                                    if not pixelProcessed:
                                        for led in range(0, self.matrixUse):
                                            for ledcolour in ledcolours:
                                                if (self.bFindValue("pixel", ledcolour) and self.value == str(
                                                        led + 1)):
                                                    ym = int(
                                                        int(led) /
                                                        self.matrixRangemax)
                                ...
                                if self.bFind("getpixel"):
                                    ...
                                    for led in range(0, self.matrixUse):
                                        if (self.bFindValue("getpixel")
                                                and self.value == str(led + 1)):
                                            ym = int(
                                                int(led) /
                                                self.matrixRangemax)
                                            xm = led % self.matrixRangemax
                        ...
                        if "pitt" in ADDON:
                            ...
                            for pin in [8, 10, 12, 16, 18, 22, 24, 26]:
                                ...
                                if bit_pattern[-(j + 1)] == '0':
                                    sghGC.pinUpdate(
                                        pin,
                                        1)  # output inverted as board is active low
                                else:
                                    sghGC.pinUpdate(pin, 0)
                                j += 1
                        else:
                        ...
                        for broadcastListLoop in broadcastList:
                            ...
                            for ym in range(0, 8):
                                for xm in range(0, 8):
                                    ...
                                    if self.bFind(
                                            "matrixon" + str(xm) + "x" + str(ym) + "y"):
                                        AdaMatrix.setPixel((xm), ym)

                                    if self.bFind(
                                            "matrixoff" + str(xm) + "x" + str(ym) + "y"):
                        ...
                        for broadcastListLoop in broadcastList:
                            ...
                            if self.bFindValue("matrixo"):
                                for ym in range(0, 8):
                                    for xm in range(0, 8):
                                        if self.bFindValue(
                                                "matrixonx" + str(xm) + "y" + str(ym)):
                                            PiMatrix.setPixel(xm, ym)
                                        if self.bFindValue(
                                                "matrixoffx" + str(xm) + "y" + str(ym)):
                    ...
                    if self.bFindValue("getcheerlights"):
                        ...
                        lookupColour = min(
                            10, max(
                                1, int(
                                    self.valueNumeric))) if self.valueIsNumeric else 1
                        # print(lookupColour)
...
logging.basicConfig(
    stream=sys.stderr,
    level=logging.DEBUG)  # default DEBUG - quiwr = INFO
