...
def angleDiff(angle1,angle2):
    return (((180 + angle1 - angle2) % 360) - 180)
...
class stepperArm():
    def __init__(self, sweepMinBase = -128, sweepMaxBase = 128, angleMinBase = -math.pi / 2.0 , angleMaxBase = math.pi / 2.0,sweepMinShoulder = 64, sweepMaxShoulder = -64, angleMinShoulder = math.pi / 4.0, angleMaxShoulder = 3 * math.pi / 4.0,sweepMinElbow = 0, sweepMaxElbow = 128, angleMinElbow = 0, angleMaxElbow = - 2.0 * math.pi / 4.0,sweepMinGripper = 75, sweepMaxGripper = 115, angleMinGripper = math.pi / 2.0, angleMaxGripper = 0):
...
class ScratchListener(threading.Thread):
    ...
    def trackerDecode(self,trackCommand):
        ...
        if isNumeric(regValue):
            ...
            robotStartDir = int(((float(sghGC.sensorDict.get('robot_direction'))) / 90.0)+ 0.5) * 90
            print "initial dir:",robotStartDir
            print "abd diff", abs(((180 + regValue + robotStartDir - int(float(sghGC.sensorDict.get('robot_direction')))) % 360) - 180)
            print "real diff",(((180 + regValue + robotStartDir - int(float(sghGC.sensorDict.get('robot_direction')))) % 360) - 180)
            while abs(((180 + regValue + robotStartDir - int(float(sghGC.sensorDict.get('robot_direction')))) % 360) - 180) > 0 :    
                print "diff", (((180 + regValue + robotStartDir - int(float(sghGC.sensorDict.get('robot_direction')))) % 360) - 180)            
                if (((180 + regValue + robotStartDir - int(float(sghGC.sensorDict.get('robot_direction')))) % 360) - 180) < 0:
                    self.robotMove(-1 * robotSpeed,robotSpeed)
                elif (((180 + regValue + robotStartDir - int(float(sghGC.sensorDict.get('robot_direction')))) % 360) - 180) >= 0:
                    self.robotMove(robotSpeed,-1 * robotSpeed)
                time.sleep(0.1)                       
            self.robotMove(0,0)  
        msgQueue.put((1, 'sensor-update "' + 'commandfinished' + '" "' + 'true' + '"'))
